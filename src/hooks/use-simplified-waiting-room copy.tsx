...
export const useSimplifiedWaitingRoom = (props: SimpleWaitingRoomProps) => {
  ...
  const refreshWaitingRoom = useCallback(async () => {
    ...
    try {
      ...
      const players: WaitingRoomPlayer[] = [];
      
      for (const player of playersData) {
        // Check if player is an AI bot
        if (player.is_bot) {
          // Add AI player (no need to fetch profile)
          players.push({
            id: player.user_id || player.id,
            name: `AI Player (${player.position})`,
            avatar: "", // You could add a default AI avatar here
        ...
        // For human players, fetch profile data
        ...
        if (profileError) {
          console.error(`Error fetching profile for player ${player.user_id}:`, profileError);
        }
        ...
        if (profileData) {
          ...
          playerAvatar = profileData.avatar_url || "";
          console.log(`Found profile for ${player.user_id}:`, profileData);
        } else {
          console.log(`No profile found for user ${player.user_id}, using fallback`);
          const shortId = player.user_id?.split("-")[0] || "unknown";
          playerName = `Player_${shortId}`;
        }
        
        if (player.user_id === user?.id) {
          console.log("This is the current user, using auth info");
          if (user.email) {
      ...
      setWaitingRoom({
        ...
        gameType: gameData.type as "solo" | "partnered",
        gameMode: gameData.mode as "practice" | "real",
      ...
      setError(`Failed to load waiting room data: ${error instanceof Error ? error.message : 'Unknown error'}`);
      setIsLoading(false);
      return false;
    }
  }, [roomId, toast, user]);
  ...
  const joinWaitingRoom = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (gameData.status !== 'waiting') {
        ...
        setError(`Game status is ${gameData.status}, not accepting new players`);
        toast({
      ...
      const userAlreadyJoined = existingPlayers?.some(p => p.user_id === user.id);
      ...
      console.error("Error joining waiting room:", error);
      setError(`Error joining room: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    } finally {
      setIsJoining(false);
    }
  ...
  const leaveWaitingRoom = useCallback(async () => {
    ...
    try {
      ...
      setError(`Error leaving room: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  ...
  const setPlayerReady = useCallback(async (isReady: boolean) => {
    ...
    try {
      ...
      console.error("Error updating ready status:", error);
      setError(`Error updating ready status: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      ...
      setError(`Error starting game: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
